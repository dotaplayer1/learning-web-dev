...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      ...
      it 'returns 7' do
        expect(game_guessing2.make_guess).to eql(7)
      end
    end
    ...
    context 'when min is 8 and max is 9' do
      ...
      it 'returns 8' do
        expect(game_guessing2.make_guess).to eql(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_guessing2) { described_class.new(0, 3, number_guessing) }

      it 'returns 1' do
        expect(game_guessing2.make_guess).to eql(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_guessing2) { described_class.new(3, 3, number_guessing) }

      it 'returns 3' do
        expect(game_guessing2.make_guess).to eql(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:answer) { double("random_number", value: 4) }
      subject(:game_guessing3) { described_class.new(0, 9, answer, 4) }
      ...
      it 'is game over' do
        expect(game_guessing3.game_over?).to be true
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      ...
      it 'is not game over' do
        expect(game_guessing3.game_over?).to be false
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:game_guessing4) { described_class.new(5, 8, number_range, 7) }

      before do
        game_guessing4.update_range
      end

      it 'updates min to the same value as max' do
        expect(game_guessing4.min).to eql(game_guessing4.max)
      end

      it 'does not update max' do
        expect(game_guessing4.max).to eql(8)
      end
    end
  end
end
