...
describe MagicSeven do
  ...
  subject(:game2) { described_class.new(20) }

  describe '#subtract_random_number' do
    context "when previous step is 100" do
      it "returns 80" do
        previous_step = 100
        result = game2.subtract_random_number(previous_step)
        expect(result).to be(80)
      end
    end
  end
  ...
  describe '#play' do
    ...
    let(:chosen_number) { 20 }
    subject(:game3) { described_class.new(chosen_number) }
